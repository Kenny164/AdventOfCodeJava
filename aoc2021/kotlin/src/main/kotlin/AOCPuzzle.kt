...
abstract class AOCPuzzle(private val dayNumber: Int) {
    ...
    private fun resourceURI(isTest: Boolean = false): URI =
        when (isTest) {
            true -> javaClass.classLoader.getResource("inputs/day" + dayNumber.toString().padStart(2, '0') + "_test.txt")
                ?.toURI() ?: throw IllegalArgumentException("Cannot find Resource: $this")
            false -> javaClass.classLoader.getResource("inputs/day" + dayNumber.toString().padStart(2, '0') + ".txt")
                ?.toURI() ?: throw IllegalArgumentException("Cannot find Resource: $this")
        }
